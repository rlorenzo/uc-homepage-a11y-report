...
function impactCard(hall, { topRuleEntry, topReachRuleEntry }) {
  if (topRuleEntry) {
    const headline = ruleFriendly(topRuleEntry[0]);
    return makeHallCard(hall, {
      accent: true,
...
function trendCard(hall, { systemTrend, prev }) {
  if (!systemTrend) {
    return makeHallCard(hall, {
...
export function renderSpotlight(ctx) {
  ...
  // Spotlight stays system-wide and is not filter-aware on purpose: it is
  // the editorial voice of the report, while filters change the data view
  // alongside it.
  hall.appendChild(impactCard(hall, ctx));
  hall.appendChild(trendCard(hall, ctx));
}
